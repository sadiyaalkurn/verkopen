...
$level1 = Yii::$app->getRequest()->getQueryParam('parent');
$pid = Yii::$app->getRequest()->getQueryParam('id');
?>
<div class="content-type-index">
	<p>
		<?php echo HTML::a('Add New', ['create', 'is_main'=>$level1, 'child'=>$pid], ['class' => 'btn btn-success create_data', 'id' => '']) ?>
	</p>
        ...
        'columns' => [
            ...
            [
                ...
                'urlCreator' => function ($action, $model, $key, $index) {
                    if ( $action === "update" ) {
                        $parent = Yii::$app->getRequest()->getQueryParam('parent');
                        $pid = Yii::$app->getRequest()->getQueryParam('id');
                        $url = Yii::$app->urlManager->createUrl(['/categories/default/update','id'=>$model->uid, 'parent'=>$parent, 'child'=>$pid]);
                        return $url;
                    } elseif($action === 'delete'){
                        $url = Yii::$app->urlManager->createUrl(['/categories/default/delete','id'=>$model->uid]);
                        return $url;
                    } elseif($action === 'view'){
                        $url = Yii::$app->urlManager->createUrl(['/categories/default/viewchild','id'=>$model->uid]);
                        return $url;
                    }  
                }
            ],
